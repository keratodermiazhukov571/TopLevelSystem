...
static void test_auth_find_by_key_match(void)
{
    ...
    /* Synthesize one user directly — portal_auth_find_by_key only reads. */
    auth.user_count = 1;
...
static void test_auth_find_by_key_ignores_empty_stored_key(void)
{
    ...
    portal_auth_registry_t auth = {0};
    auth.user_count = 1;
    /* User exists but has no api_key set — must never match any lookup,
     * especially not a caller who accidentally sent an empty token. */
    snprintf(auth.users[0].username, sizeof(auth.users[0].username),
             "keyless");
